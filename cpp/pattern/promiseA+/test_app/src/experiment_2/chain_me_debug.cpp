...
public:
    Promise() { std::cout << "[Promise<void>] Constructor: this=" << this << std::endl; }
    ~Promise() { std::cout << "[Promise<void>] Destructor: this=" << this << std::endl; }
    void resolve() {
        std::lock_guard<std::mutex> lock(callback_mutex_);
        std::cout << "[Promise<void>] resolve: this=" << this << ", state=" << (int)state_ << std::endl;
        if (state_ != State::Pending) return;
        ...
        for (auto& cb : thens) {
            std::cout << "[Promise<void>] resolve: enqueue then-callback, this=" << this << std::endl;
            EventLoop::getInstance().enqueue(cb);
        }
        for (auto& cb : catchs) {
            std::cout << "[Promise<void>] resolve: enqueue catch-callback, this=" << this << std::endl;
            EventLoop::getInstance().enqueue(cb);
        }
    }
    void reject(std::exception_ptr e) {
        std::lock_guard<std::mutex> lock(callback_mutex_);
        std::cout << "[Promise<void>] reject: this=" << this << ", state=" << (int)state_ << std::endl;
        if (state_ != State::Pending) return;
        ...
        catch_callbacks_.clear();
        for (auto& cb : catchs) {
            std::cout << "[Promise<void>] reject: enqueue catch-callback, this=" << this << std::endl;
            EventLoop::getInstance().enqueue(cb);
        }
        for (auto& cb : thens) {
            std::cout << "[Promise<void>] reject: enqueue then-callback, this=" << this << std::endl;
            EventLoop::getInstance().enqueue(cb);
        }
    }
    template <typename F>
    auto then(F cb) {
        ...
        auto newPromise = std::make_shared<Promise<ReturnType>>();
        auto self = this->shared_from_this();
        std::cout << "[Promise<void>::then] Attach then-callback: this=" << this << ", newPromise=" << newPromise.get() << std::endl;
        auto exec = [self, cb, newPromise]() {
            std::cout << "[Promise<void>::then] Execute then-callback: self=" << self.get() << ", newPromise=" << newPromise.get() << std::endl;
    ...
    auto catchError(F cb) {
        ...
        auto newPromise = std::make_shared<Promise<ReturnType>>();
        auto self = this->shared_from_this();
        std::cout << "[Promise<void>::catchError] Attach catch-callback: this=" << this << ", newPromise=" << newPromise.get() << std::endl;
        auto exec = [self, cb, newPromise]() {
            std::cout << "[Promise<void>::catchError] Execute catch-callback: self=" << self.get() << ", newPromise=" << newPromise.get() << std::endl;
        ...
        };
        {
            ...
            if (state_ == State::Pending && !callbacks_moved_) {
                // Để đúng chuẩn, chỉ push vào catch_callbacks_ (không push vào then_callbacks_)
                catch_callbacks_.push_back(exec);
            } else EventLoop::getInstance().enqueue(exec);
        }
        return newPromise;
    ...
    auto finally(F cb) {
        ...
        std::cout << "[Promise<void>::finally] Attach finally-callback: this=" << this << ", newPromise=" << newPromise.get() << std::endl;
        auto exec = [self, cb, newPromise]() {
            std::cout << "[Promise<void>::finally] Execute finally-callback: self=" << self.get() << ", newPromise=" << newPromise.get() << ", state=" << (int)self->state_ << std::endl;
            try { cb(); } catch (...) {}
        ...
        };
        {
            std::lock_guard<std::mutex> lock(callback_mutex_);
            if (state_ == State::Pending && !callbacks_moved_) {
                // Để đúng chuẩn, chỉ push vào then_callbacks_ (không push vào cả hai)
                then_callbacks_.push_back(exec);
            } else EventLoop::getInstance().enqueue(exec);
        }
        return newPromise;
