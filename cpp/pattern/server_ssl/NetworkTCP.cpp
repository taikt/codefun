...
bool WriteDataSSL(SSL* ssl, unsigned char* data, size_t length) {
    ...
    mMutex.lock();
    rc = SSL_write(ssl, (char*)data, length);
    mMutex.unlock();

    if (rc != length) {
        printf("cannot write SSL: rc=%d\n",rc);
        return false;
    }
    else return true;
}
...
bool ReadDataSSL(SSL* ssl, unsigned char* data) {
    int len;
    while ((len = SSL_read(ssl, data, BUFFER_SIZE)) != 0) {
        if (len < 0) {
...
void serialize_request(RequestMsg &msg, unsigned char* data) {
    ...
    *p = msg.pw_len;
    p++;
    memcpy(p, &(msg.session_id), 4);
    //*p = msg->session_id;
    p += 4; // 4 byte for sessionId
    memcpy(p, &(msg.payload_len), 4);
    //*p = msg->payload_len;
    p += 4; // 4 byte for payload length

    // serialize payload
    int i = 0;
    while (i < msg.payload_len) {
        *p = msg.payload[i];
        i++;
        p++;
    }
...
void deserialize_request(RequestMsg* msg, unsigned char* data) {
    ...
    msg->pw_len = *p;
    p++;
    memcpy(&(msg->session_id), p, 4);    
    p += 4;
    memcpy(&(msg->payload_len), p, 4);
    p += 4;
