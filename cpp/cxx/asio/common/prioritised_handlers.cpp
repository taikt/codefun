// https://www.boost.org/doc/libs/1_53_0/doc/html/boost_asio/example/invocation/prioritised_handlers.cpp
...
// g++ prioritised_handlers.cpp -lboost_system -lboost_thread
...
void low_priority_handler(int a)
{
  std::cout << "Low priority handler:"<<a<<"\n";
}

void low2_priority_handler()
{
  std::cout << "Low2 priority handler\n";
}

void low10_priority_handler()
{
  std::cout << "Low10 priority handler\n";
}
...
int main()
{
  ...
  timer1.async_wait(pri_queue.wrap(10, middle_priority_handler));

  //boost::asio::deadline_timer timer2(io_service, boost::posix_time::seconds(3));
  //timer2.async_wait(pri_queue.wrap(0, low_priority_handler));

  boost::asio::deadline_timer timer3(io_service, boost::posix_time::seconds(1));
  timer3.async_wait(pri_queue.wrap(100, high_priority_handler));
...
#if 1
  pri_queue.add(1,boost::bind(&low_priority_handler,2));
  pri_queue.add(5,low2_priority_handler);
  pri_queue.add(2,low10_priority_handler);
  pri_queue.execute_all();
#endif
