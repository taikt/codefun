...
def generate_synthetic_example(rule_id, is_violation):
    ...
    elif rule["rule_id"] == "MISRA C++ 5-0-1":
...
def label_function(code, label):
    if label == "MISRA C++ 3-1-1":
        is_violation = "inline" in code
        suggestion = "Xóa từ khóa inline hoặc di chuyển khai báo inline vào tệp tiêu đề." if is_violation else ""
    elif label == "MISRA C++ 5-0-1":
        is_violation = "&&" in code or "||" in code
        suggestion = "Tách biểu thức logic thành các câu lệnh if lồng nhau." if is_violation else ""
    else:
        is_violation = False
        suggestion = ""
    rule = next(r for r in MISRA_RULES if r["labels"] == label)
    return {
        "code": code,
        "labels": rule["labels"],
        "description": rule["description"],
...
# Hàm tạo tập dữ liệu
def create_misra_dataset(num_synthetic=1000, cpp_files=None, output_file="/content/drive/MyDrive/misra_dataset.json"):
    dataset = []
        ...
        for cpp_file in cpp_files:
            ...
            for func in functions:
                for rule in MISRA_RULES:
                    labeled = label_function(func["code"], rule["labels"])
...
if __name__ == "__main__":
    ...
    create_misra_dataset(num_synthetic=1000, cpp_files=cpp_files, output_file="/content/drive/MyDrive/misra_dataset.json")
