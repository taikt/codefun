...
// Lấy hàm getReportDirectory từ extension
...
export function registerCheckMultipleFilesLGEDVCommand(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand('lgedv.checkMultipleFilesLGEDV', async () => {
    ...
    const outputChannel = vscode.window.createOutputChannel('CodeGuard - LGEDV Multi-file');
    outputChannel.clear();
    outputChannel.show(true);
    outputChannel.appendLine(`🔎 Found ${cppFiles.length} C++ files. Starting LGEDV analysis...`);
    const rulesContent = await loadRules('LGEDV', outputChannel);
    if (!rulesContent || rulesContent.trim().length === 0) {
      outputChannel.appendLine('❌ No LGEDV rules loaded - analysis cannot proceed');
      vscode.window.showErrorMessage('Failed to load LGEDV rule files');
      return;
    }
    ...
    // Lấy đúng defectReportDir như checkLGEDV
    ...
    for (let i = 0; i < cppFiles.length; ++i) {
      ...
      try {
        ...
        const prompt = createFocusedPrompt(codeContent, rulesContent, 'LGEDV');
        ...
        await saveReport(filePath, 'LGEDV', response, codeContent, selectedModel.name, timestamp, outputChannel, defectReportDir);
    ...
    outputChannel.appendLine(`\n🎉 LGEDV multi-file analysis complete. ${successCount}/${cppFiles.length} files analyzed.`);
    vscode.window.showInformationMessage(`LGEDV multi-file analysis complete. ${successCount}/${cppFiles.length} files analyzed.`);
  });
