...
export function registerCheckMultipleFilesCERTCommand(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand('lgedv.checkMultipleFilesCERT', async () => {
    ...
    const outputChannel = vscode.window.createOutputChannel('CodeGuard - CERT Multi-file');
    outputChannel.clear();
    outputChannel.show(true);
    outputChannel.appendLine(`🔎 Found ${cppFiles.length} C++ files. Starting CERT analysis...`);
    const rulesContent = await loadRules('CERT', outputChannel);
    if (!rulesContent || rulesContent.trim().length === 0) {
      outputChannel.appendLine('❌ No CERT rules loaded - analysis cannot proceed');
      vscode.window.showErrorMessage('Failed to load CERT rule files');
      return;
    }
    ...
    for (let i = 0; i < cppFiles.length; ++i) {
      ...
      try {
        ...
        const prompt = createFocusedPrompt(codeContent, rulesContent, 'CERT');
        ...
        await saveReport(filePath, 'CERT', response, codeContent, selectedModel.name, timestamp, outputChannel, defectReportDir);
    ...
    outputChannel.appendLine(`\n🎉 CERT multi-file analysis complete. ${successCount}/${cppFiles.length} files analyzed.`);
    vscode.window.showInformationMessage(`CERT multi-file analysis complete. ${successCount}/${cppFiles.length} files analyzed.`);
  });
