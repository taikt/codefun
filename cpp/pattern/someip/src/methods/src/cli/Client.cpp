...
#include <v1/taikt/myInterfaceProxy.hpp>
#include <sys/types.h>
#include <unistd.h>
...
void recv_cb(const CommonAPI::CallStatus& callStatus,
             const myInterface::stdErrorTypeEnum& methodError,
             const int32_t& y1,
             const std::string& y2) {
    ...
    std::cout << "   callStatus: " << ((callStatus == CommonAPI::CallStatus::SUCCESS) ? "SUCCESS" : "NO_SUCCESS")
                    << std::endl;
    std::cout << "   error: "
                    << ((methodError == myInterface::stdErrorTypeEnum::NO_FAULT) ? "NO_FAULT" :
                                    "MY_FAULT") << std::endl;
...
int main() {
    ...
    std::string instance = "MyInstanceTesting";

    std::shared_ptr<myInterfaceProxy<>> myProxy = runtime->buildProxy < myInterfaceProxy > (domain, instance, "client-sample1");

    std::cout<<"taikt test\n";
    if (myProxy == nullptr) {
        std::cout<<"proxy is null\n";
    } else {
        ...
        std::cout<<"taikt proxy is available1\n";
        ...
        while (true) {
            ...
            std::cout << "   callStatus: " << ((callStatus == CommonAPI::CallStatus::SUCCESS) ? "SUCCESS" : "NO_SUCCESS")
                    << std::endl;
            std::cout << "   error: "
                    << ((methodError == myInterface::stdErrorTypeEnum::NO_FAULT) ? "NO_FAULT" : "MY_FAULT")
                    << std::endl;
            ...
            std::function<
                            void(const CommonAPI::CallStatus&,
                                const myInterface::stdErrorTypeEnum&,
                                const int32_t&,
